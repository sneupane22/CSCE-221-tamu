...
private:
  ...
  int listSize;
  T obj;
  DListNode *prev, *next, *representative; //REPRESENTATIVE SHOULD POINT OT HEAD OF LIST 
...
public:
  ...
  int getKey() { return key; }
    void setKey(int k) {this->key = k;} // set key function
  DListNode<T>* getRepresentative() const {return representative;}
  DListNode<T>* getTrailer() const{return trailer;}
  void setRepresentative(DListNode* rep) {this->representative = rep;}
  void setTrailer(DListNode* trail) {this->trailer = trail; }
